...
class PolygonDatabase:
    ...
    def update_stored_heatmap_boundaries(self, heatmap_id, boundary_updates):
        """
        Update stored heatmap with boundary snapping adjustments for seamless connections
        
        Parameters:
        -----------
        heatmap_id : int
            ID of the stored heatmap to update
        boundary_updates : dict
            Dictionary with boundary adjustments: {'east': 172.123, 'west': 171.987, etc.}
        ...
        try:
            ...
            with self.engine.connect() as conn:
                result = conn.execute(text("""
                    SELECT geojson_data FROM stored_heatmaps WHERE id = :heatmap_id
                """), {'heatmap_id': heatmap_id})
                
                row = result.fetchone()
                if not row or not row[0]:
                    print(f"No GeoJSON data found for heatmap ID {heatmap_id}")
                    return False
                
                geojson_data = json.loads(row[0])
                
                # Apply boundary snapping to the GeoJSON features
                snap_threshold_km = 3.0
                total_snapped_vertices = 0
                
                print(f"🔧 UPDATING STORED HEATMAP {heatmap_id}: Applying boundary snapping to {len(geojson_data.get('features', []))} features...")
                ...
                for feature in geojson_data.get('features', []):
                    if 'geometry' in feature and feature['geometry']['type'] == 'Polygon':
                        ...
                        for coord_idx in range(len(coords) - 1):  # Skip last coord (duplicate of first)
                            ...
                            for boundary_type, boundary_value in boundary_updates.items():
                                if boundary_value is None:
                                    continue
                                    
                                if boundary_type == 'east':
                                    ...
                                    if distance_km <= snap_threshold_km:
                                        ...
                                        if total_snapped_vertices <= 5:  # Log first few snaps
                                            print(f"🎯 EAST BOUNDARY UPDATE: vertex ({original_lon:.6f}, {original_lat:.6f}) → ({boundary_value:.6f}, {original_lat:.6f}), distance: {distance_km:.3f}km")
                                        break
                                        
                                elif boundary_type == 'west':
                                    from utils import get_distance
                                    distance_km = get_distance(original_lat, original_lon, original_lat, boundary_value)
                                    if distance_km <= snap_threshold_km:
                                        ...
                                        total_snapped_vertices += 1
                                        if total_snapped_vertices <= 5:  # Log first few snaps
                                            print(f"🎯 WEST BOUNDARY UPDATE: vertex ({original_lon:.6f}, {original_lat:.6f}) → ({boundary_value:.6f}, {original_lat:.6f}), distance: {distance_km:.3f}km")
                                        break
                ...
                print(f"🔧 BOUNDARY UPDATE COMPLETE: {total_snapped_vertices} vertices updated in stored heatmap {heatmap_id}")
                
                # Update the database with modified GeoJSON
                conn.execute(text("""
                    UPDATE stored_heatmaps 
                    SET geojson_data = :geojson_data, updated_at = :updated_at
                    WHERE id = :heatmap_id
                """), {
                    'heatmap_id': heatmap_id,
            ...
            print(f"Error updating stored heatmap boundaries: {e}")
            return False
