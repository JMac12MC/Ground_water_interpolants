...
with main_col1:
    st.header("Interactive Groundwater Map")
    
    # Initialize clean state
    if 'current_analysis_point' not in st.session_state:
        st.session_state.current_analysis_point = None
    if 'analysis_data_ready' not in st.session_state:
        st.session_state.analysis_data_ready = False
    if 'click_position' not in st.session_state:
        st.session_state.click_position = None
    
    # Always create a clean, fully interactive map
    default_center = [-43.5320, 172.6306]
    default_zoom = 8
