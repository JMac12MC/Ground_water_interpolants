...
def generate_single_heatmap(wells_data, center_point, search_radius, interpolation_method, polygon_db, soil_polygons=None, new_clipping_polygon=None, heatmap_id_prefix="single"):
    """
    Generate a single heatmap at the specified center point.
    
    Args:
        wells_data: DataFrame with wells data
        center_point: [lat, lon] coordinates for heatmap center
        search_radius: radius in km for well filtering
        interpolation_method: 'yield' or 'ground_water_level'
        polygon_db: Database instance for storing heatmaps
        soil_polygons: Optional soil polygon data for clipping
        new_clipping_polygon: Optional NEW clipping polygon for boundaries
        heatmap_id_prefix: Prefix for the heatmap ID
        
    Returns:
        tuple: (success_boolean, heatmap_id) or (False, error_message)
    """
    ...
    print(f"🎯 Generating single heatmap at ({lat:.6f}, {lon:.6f})")
    
    try:
        # Filter wells within search radius
        wells_within_radius = []
        for idx, row in wells_data.iterrows():
            if 'latitude' in row and 'longitude' in row:
                well_lat, well_lon = row['latitude'], row['longitude']
                distance = get_distance(lat, lon, well_lat, well_lon)
                if distance <= search_radius:
                    wells_within_radius.append(row)
        
        filtered_wells_df = wells_data.loc[wells_data.index.isin([w.name for w in wells_within_radius])]
        
        if len(filtered_wells_df) == 0:
            return False, f"No wells found within {search_radius}km radius"
        
        print(f"📊 Found {len(filtered_wells_df)} wells within {search_radius}km radius")
        
        # Generate interpolation using the standard interface
        ...
        if geojson_data and 'features' in geojson_data and len(geojson_data['features']) > 0:
            ...
            method_name = interpolation_method.replace('_kriging', '_kriging')
            heatmap_id = f"{method_name}_{heatmap_id_prefix}_{lat:.6f}_{lon:.6f}"
            ...
            success = polygon_db.store_heatmap(
                ...
                colormap_metadata={
                    ...
                    'wells_count': len(filtered_wells_df)
                }
            )
            
            if success:
                print(f"✅ Stored heatmap: {heatmap_id}")
                return True, heatmap_id
            else:
                return False, f"Failed to store heatmap in database"
        else:
            return False, "Interpolation generated no valid features"
